...
export default function AnimatedBackground() {
  ...
  useEffect(() => {
    ...
    function drawSweep(t) {
      ...
      grad.addColorStop(0.05, 'rgba(255,107,61,0.025)')
      grad.addColorStop(0.1, 'rgba(255,107,61,0)')
    ...
    function drawGrid(t) {
      ...
      blips.forEach((b) => {
        ...
        ctx.fillStyle = `rgba(255,107,61,${a * 0.55})`
        ctx.fill()
        ctx.beginPath()
        ctx.arc(bx, by, 5 + a * 5, 0, Math.PI * 2)
        ctx.strokeStyle = `rgba(255,107,61,${a * 0.25})`
        ctx.lineWidth = 1
        ctx.stroke()
      })
    }
    ...
    function drawParticles() {
      ...
      particles.forEach((p) => {
        ...
        if (p.isAmber) {
          ctx.fillStyle = `rgba(255,107,61,${p.alpha})`
        } else {
          ctx.fillStyle = `rgba(236,232,223,${p.alpha})`
        }
        ctx.fill()
      })
            ...
            if (pDist < 85) {
              ...
              ctx.strokeStyle =
                p1.isAmber || p2.isAmber
                  ? `rgba(255,107,61,${lineAlpha})`
                  : `rgba(236,232,223,${lineAlpha})`
              ctx.lineWidth = 0.8
              ctx.stroke()
            }
          }
