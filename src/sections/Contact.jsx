...
export default function Contact() {
  ...
  function handleSubmit(e) {
    ...
    // form gives real feedback instead of silently doing nothing.
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        ...
        >
          ...
          <form
            ...
            className="hud-frame border border-line bg-surface p-8 w-full md:w-[380px] shrink-0 flex flex-col gap-5 relative overflow-hidden"
          >
            <AnimatePresence mode="wait">
              {status === 'sent' ? (
                ...
                >
                  <div className="w-12 h-12 rounded-full border border-amber flex items-center justify-center">
                    <Check className="w-5 h-5 text-amber" />
                  </div>
                  <div>
                    <div className="mono-label text-xs text-amber mb-1">Transmission Sent</div>
                    <p className="text-sm text-paper-dim">We've logged your message and will respond shortly.</p>
                  </div>
                  <button
                    type="button"
                    onClick={handleReset}
                    className="mono-label text-[10px] text-paper-dim hover:text-amber transition-colors mt-1"
                  >
                    Send another
                  </button>
                </motion.div>
                ...
                >
                  ...
                  <MagneticButton
                    ...
                    className="btn-shine mono-label text-xs bg-amber text-graphite px-6 py-3.5 hover:bg-paper transition-colors mt-2 flex items-center justify-center gap-2 disabled:opacity-60"
                  >
                    {status === 'sending' ? 'Transmitting…' : 'Send Transmission'}
                    {status !== 'sending' && <ArrowUpRight className="w-3.5 h-3.5" />}
                  </MagneticButton>
                </motion.div>
              )}
            </AnimatePresence>
